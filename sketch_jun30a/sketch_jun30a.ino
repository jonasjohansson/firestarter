...
const uint8_t redValue = 255;
const uint8_t greenValue = 64;
const uint8_t blueValue = 0;
...
void loop() {
  ...
  float phase = static_cast<float>(currentTime % fadeDuration) / fadeDuration;
  uint8_t masterValue = static_cast<uint8_t>((sin(phase * 2 * PI) + 1) * 127.5);

  setDmx(masterChannel, masterValue);
  setDmx(redChannel, redValue);
  setDmx(greenChannel, greenValue);
  setDmx(blueChannel, blueValue);
